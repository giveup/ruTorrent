...
{
	plugin.reqId1 = theRequestManager.addRequest("trt", theRequestManager.map("d.custom=")+"chk-state",function(hash,torrent,value)
	{
		torrent.chkstate = value;
		if(torrent.chkstate==4)	// STE_DELETED
			torrent.state |= dStatus.error;
	});
	plugin.reqId2 = theRequestManager.addRequest("trt", theRequestManager.map("d.custom=")+"chk-time",function(hash,torrent,value)
	{
